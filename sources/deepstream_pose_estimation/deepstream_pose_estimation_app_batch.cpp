...
static gchar *_input_list_file = NULL;
static gchar *_output_root = NULL;
...
gint frame_number = 0;
std::vector<std::string> g_input_list;
size_t g_current_input_idx = 0;
...
static bool load_input_list_file(const char *filename)
{
  std::ifstream in(filename);
  if (!in.is_open()) {
    g_printerr("Cannot open input list file %s. Exiting...\n", filename);
    return false;
  }
  ...
  while (std::getline(in, line)) {
    ...
    g_input_list.push_back(line);
  }

  if (g_input_list.empty()) {
    g_printerr("Input list file %s is empty. Exiting...\n", filename);
    return false;
  }
  return true;
}

static std::string sanitize_stem(const std::string &input_uri)
{
  ...
  if (stem.empty()) stem = "pose";
  return stem;
}

static bool open_pose_output_for_current_input()
{
  if (_pose_file) {
    fclose(_pose_file);
    _pose_file = NULL;
  ...
  if (!_pose_filename && !_output_root)
    return true;

  std::string out_path;

  if (_output_root && !g_input_list.empty()) {
    std::string input_uri = _input ? _input : g_input_list[g_current_input_idx];
    std::string input_path = input_uri;

    if (input_path.rfind("file://", 0) == 0) {
      input_path = input_path.substr(7);
    }

    const std::string input_root = "/workspace/dataset/videos_mp4/";
    std::string relative_path;

    if (input_path.rfind(input_root, 0) == 0) {
      relative_path = input_path.substr(input_root.size());
    } else {
      size_t slash = input_path.find_last_of("/\\");
      relative_path = (slash == std::string::npos)
          ? input_path
          : input_path.substr(slash + 1);
    }

    size_t dot = relative_path.find_last_of('.');
    if (dot != std::string::npos) {
      relative_path = relative_path.substr(0, dot);
    }
    relative_path += ".json";

    out_path = std::string(_output_root);
    if (!out_path.empty() && out_path.back() != '/')
      out_path += "/";
    out_path += relative_path;

  } else if (_pose_filename) {
    out_path = _pose_filename;
  } else {
    return true;
  }
  ...
  if (p.has_parent_path()) {
    ...
    if (ec) {
      g_printerr("Cannot create output directory for %s. Exiting...\n", out_path.c_str());
      return false;
    }
  }

  _pose_file = fopen(out_path.c_str(), "wt");
  if (!_pose_file) {
  ...
  g_print("Saving pose JSON to: %s\n", out_path.c_str());
  return true;
}

static void close_pose_output()
{
  if (_pose_file) {
    fsetpos(_pose_file, &g_fp_25_pos);
    fprintf(_pose_file, "]\n");
...
{
  if (_input_list_file) {
    if (!load_input_list_file(_input_list_file))
      return false;
  } else if (!_input) {
    g_printerr("--input or --input-list option is not specified. Exiting...\n");
    return false;
  } else {
  ...
  if (_input_list_file && _pose_filename && !_output_root) {
    g_printerr("When using --input-list with --save-pose, also provide --output-root so each input gets its own JSON file. Exiting...\n");
    return false;
  }
...
int main(int argc, char *argv[])
{
  ...
  //---Parse command line options---
  {
    const GOptionEntry entries[] = {
      { "input-list", 0, 0, G_OPTION_ARG_STRING, &_input_list_file,
        "Text file containing one input URI per line.", "FILE" }
      ,
      { "output-root", 0, 0, G_OPTION_ARG_STRING, &_output_root,
        "Root directory where per-video JSON files will be written.", "DIR" }
      ,
...
"(https://docs.nvidia.com/metropolis/deepstream/dev-guide/text/DS_plugin_gst-nvtracker.html) section for details.",
  ...
  if (!g_input_list.empty())
    run_inputs = g_input_list;
  else
    run_inputs.push_back(_input);

  for (g_current_input_idx = 0; g_current_input_idx < run_inputs.size(); ++g_current_input_idx) {
    _input = g_strdup(run_inputs[g_current_input_idx].c_str());
    frame_number = 0;
    g_filter_pose25d.clear();
    m_filterRootDepth.reset();

    if (!open_pose_output_for_current_input()) {
      g_free(_input);
      return -1;
    }
    ...
    gchar *tracker_ll_config_file = NULL, *tracker_ll_lib_file = NULL;
    gint tracker_width = 0, tracker_height = 0;
    gboolean enable_batch_process = FALSE;

    tracker_width = g_key_file_get_integer (key_file, CONFIG_GROUP_TRACKER,
        CONFIG_GROUP_TRACKER_WIDTH, &error);
    CHECK_ERROR (error);
    tracker_height = g_key_file_get_integer (key_file, CONFIG_GROUP_TRACKER,
        CONFIG_GROUP_TRACKER_HEIGHT, &error);
    CHECK_ERROR (error);
    tracker_ll_config_file = g_key_file_get_string (key_file, CONFIG_GROUP_TRACKER,
        CONFIG_GROUP_TRACKER_LL_CONFIG_FILE, &error);
    CHECK_ERROR (error);
    tracker_ll_lib_file = g_key_file_get_string (key_file, CONFIG_GROUP_TRACKER,
        CONFIG_GROUP_TRACKER_LL_LIB_FILE, &error);
    CHECK_ERROR (error);
    enable_batch_process = g_key_file_get_boolean (key_file, CONFIG_GROUP_TRACKER,
    ...
    g_object_set (G_OBJECT (tracker),
        ...
        "enable_batch_process", enable_batch_process,
        NULL);

    g_key_file_free (key_file);
    g_free (tracker_ll_config_file);
    g_free (tracker_ll_lib_file);

    g_object_set(G_OBJECT(pgie),
        "config-file-path", PGIE_CONFIG_FILE,
        NULL);

    if (_tracker && !strcmp(_tracker, "accuracy")) {
      g_object_set(G_OBJECT(tracker), "tracking-surface-type", 0, NULL);
    }
    ...
    if (_print_fps) {
      g_object_set (G_OBJECT(nvdslogger), "fps-measurement-interval-sec", _fps_interval, NULL);
    } else {
      g_object_set (G_OBJECT(nvdslogger), "silent", TRUE, NULL);
    }
  ...
  if (_nvmsgbroker_conn_str) {// Publish metadata to a broker as well
    ...
    g_print("Now playing [%zu/%zu]: %s\n", g_current_input_idx + 1, run_inputs.size(), _input);
