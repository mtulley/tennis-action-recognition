// This file is derived from NVIDIA DeepStream Reference Applications:
...
static gchar *_input_dir = NULL;   // batch mode: input folder containing videos
static gchar *_output_dir = NULL;  // batch mode: output folder for JSON skeletons
...
// ---------------------------------------------------------------------------
// Batch-mode helpers  (C++14 / POSIX — no std::filesystem required)
// ---------------------------------------------------------------------------

/** Known video file extensions (lower-case). Add more as needed. */
static const std::vector<std::string> VIDEO_EXTENSIONS = {
  ".mp4", ".mkv", ".avi", ".mov", ".ts", ".h264", ".h265", ".hevc",
...
static bool path_is_regular_file(const std::string &p) {
  struct stat st;
...
static bool mkdir_p(const std::string &path) {
  ...
  while (start < path.size()) {
    ...
    if (slash == std::string::npos) {
      ...
      component = path.substr(start, slash - start);
      start = slash + 1;
    }
    if (component.empty()) continue;
    cur = path_join(cur, component);
    if (!path_is_dir(cur)) {
...
static bool is_video_file(const std::string &path) {
  if (!path_is_regular_file(path)) return false;
  std::string ext = str_tolower(path_extension(path));
  for (const auto &ve : VIDEO_EXTENSIONS) {
...
/**
 * Recursively collect all video files under root_dir.
 * Returns a sorted list of absolute/relative paths.
 */
static void collect_video_files_impl(const std::string &dir,
  ...
  for (const auto &sub : subdirs) {
    collect_video_files_impl(sub, result);
  }
}

static std::vector<std::string>
collect_video_files(const std::string &root_dir) {
  std::vector<std::string> result;
  if (!path_is_dir(root_dir)) {
    g_printerr("Input directory does not exist or is not a directory: %s\n",
               root_dir.c_str());
    return result;
  }
  collect_video_files_impl(root_dir, result);
...
/**
 * Given:
 *   input_dir  = "videos_mp4"
 *   output_dir = "skeletons_json"
 *   video_path = "videos_mp4/backhand/shot1.mp4"
 * Returns:      "skeletons_json/backhand/shot1.json"
 *
 * The directory component is created if it does not yet exist.
 */
static std::string
derive_output_json_path(const std::string &input_dir,
                        const std::string &output_dir,
                        const std::string &video_path) {
  // Normalise trailing slashes for prefix stripping
  std::string in_prefix = input_dir;
  if (!in_prefix.empty() && in_prefix.back() != '/') in_prefix += '/';

  std::string rel = video_path;
  if (rel.substr(0, in_prefix.size()) == in_prefix) {
    rel = rel.substr(in_prefix.size());
  }
  ...
  std::string out_path = path_join(output_dir, rel);

  // Create parent directory
  size_t last_slash = out_path.rfind('/');
  if (last_slash != std::string::npos) {
    std::string parent = out_path.substr(0, last_slash);
    if (!mkdir_p(parent)) {
      g_printerr("Warning: could not create output directory: %s\n",
                 parent.c_str());
    }
  }
  return out_path;
...
{
  // ---- Validate that the user chose one mode: single-file OR batch-dir ----
  const bool single_mode = (_input != NULL);
  const bool batch_mode  = (_input_dir != NULL);

  if (!single_mode && !batch_mode) {
    g_printerr("Either --input or --input-dir must be specified. Exiting...\n");
    return false;
  }
  if (single_mode && batch_mode) {
    g_printerr("--input and --input-dir are mutually exclusive. Exiting...\n");
    return false;
  }
  ...
  if (batch_mode) {
    if (!_output_dir) {
      g_printerr("--output-dir must be specified when using --input-dir. Exiting...\n");
      return false;
    }
    // Create the root output directory if it does not already exist.
    if (!mkdir_p(_output_dir)) {
      g_printerr("Cannot create output directory %s. Exiting...\n", _output_dir);
      return false;
    }
  }
  ...
  if (_pose_filename) {
    // Validate the path is writable by attempting a quick open.
    FILE *test = fopen(_pose_filename, "wt");
    if (!test) {
      g_printerr("Cannot open file %s. Exiting...\n", _pose_filename);
      return false;
    }
    fclose(test);
    // The actual file will be opened inside run_pipeline so JSON bookkeeping
    // (the leading "[" and the fgetpos) is done in one place.
  }
...
// Forward declaration — defined after main()
static int run_pipeline(const char *input_uri);
  ...
  //---Parse command line options---
  {
    const GOptionEntry entries[] = {
      ...
      {"input", 0, 0, G_OPTION_ARG_STRING, &_input,
        "[Required, single-file mode] Input video address in URI format by starting \
with \"rtsp://\" or \"file://\".",
        NULL}
      ,
...
pose3d in JSON format.",
        NULL}
      ,
      {"input-dir", 0, 0, G_OPTION_ARG_STRING, &_input_dir,
        "[Required, batch mode] Path to a folder containing input videos. \
All video files are processed recursively, preserving subfolder structure.",
        NULL}
      ,
      {"output-dir", 0, 0, G_OPTION_ARG_STRING, &_output_dir,
        "[Required with --input-dir] Path to the output folder where JSON \
skeleton files will be written, mirroring the input subfolder structure.",
        NULL}
      ,
...
"(https://docs.nvidia.com/metropolis/deepstream/dev-guide/text/DS_plugin_gst-nvtracker.html) section for details.",
  ...
  // ---- Batch mode: iterate all videos in the input directory ----
  if (_input_dir) {
    std::string in_root(_input_dir);
    std::string out_root(_output_dir);

    auto videos = collect_video_files(in_root);
    if (videos.empty()) {
      g_printerr("No video files found under %s. Exiting.\n", _input_dir);
      return -1;
    }

    g_print("Found %zu video(s) to process.\n", videos.size());
    int success_count = 0;

    for (size_t idx = 0; idx < videos.size(); idx++) {
      const std::string &vpath = videos[idx];
      std::string uri = "file://" + vpath;
      std::string json_out = derive_output_json_path(in_root, out_root, vpath);

      g_print("\n[%zu/%zu] Processing: %s\n", idx + 1, videos.size(), vpath.c_str());
      g_print("          Output JSON: %s\n", json_out.c_str());

      // Reset per-video state
      g_filter_pose25d.clear();
      m_filterRootDepth.reset();
      frame_number = 0;
      _quit = FALSE;
      _cintr = FALSE;

      // Open the pose output file for this video
      _pose_file = fopen(json_out.c_str(), "wt");
      if (!_pose_file) {
        g_printerr("Cannot open output file %s. Skipping.\n", json_out.c_str());
        continue;
      }
      fprintf(_pose_file, "[");
      fgetpos(_pose_file, &g_fp_25_pos);

      int ret = run_pipeline(uri.c_str());
      ...
      if (ret == 0) {
        ...
        g_printerr("          Pipeline failed for %s (ret=%d).\n", vpath.c_str(), ret);
      }
    }

    g_print("\nBatch complete: %d/%zu videos succeeded.\n", success_count, videos.size());
    return (success_count == (int)videos.size()) ? 0 : -1;
  }

  // ---- Single-file mode (original behaviour) ----
  return run_pipeline(_input);
}

// ---------------------------------------------------------------------------
// run_pipeline: builds and runs the DeepStream pipeline for one input URI.
// _pose_file / _output / _tracker etc. must already be configured by the
...
static int run_pipeline(const char *input_uri)
{
  ...
  GstElement *tee = NULL, *msgbroker = NULL, *msgconv = NULL;
  GstBus *bus = NULL;
  guint bus_watch_id;
  ...
  /* Source element for reading from the file/uri */
  {
    ...
    source = create_source_bin(0, const_cast<char*>(input_uri));
    if (!source) {
  ...
  // In single-file mode, open the pose output file here (batch mode opens it externally).
  bool single_file_pose_opened = false;
  if (!_input_dir && _pose_filename && !_pose_file) {
    _pose_file = fopen(_pose_filename, "wt");
    if (!_pose_file) {
      g_printerr("Cannot open pose file %s. Exiting.\n", _pose_filename);
      return -1;
    }
    fprintf(_pose_file, "[");
    fgetpos(_pose_file, &g_fp_25_pos);
    single_file_pose_opened = true;
  }
  ...
  // Close pose file only if we opened it in single-file mode.
  // In batch mode the caller manages the file.
  if (single_file_pose_opened && _pose_file) {
    fsetpos(_pose_file, &g_fp_25_pos);
    fprintf(_pose_file, "]\n");
    fclose(_pose_file);
    _pose_file = NULL;
